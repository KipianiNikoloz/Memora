...
describe("AI librarian route", () => {
  ...
  it("returns AI task results from the runtime", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/ai/librarian", {
        method: "POST",
        body: JSON.stringify({
          task: "reflect",
          entry: seedEntries[0],
  ...
  it("rejects requests with invalid entry contract values", async () => {
    vi.doMock("@/lib/ai-runtime", () => ({
      runAiLibrarianTask: vi.fn(),
    }));
    const { POST } = await import("@/app/api/ai/librarian/route");
  ...
  it("rejects invalid runtime response payloads", async () => {
    vi.doMock("@/lib/ai-runtime", () => ({
    ...
    await expect(response.json()).resolves.toEqual({ error: "Invalid AI librarian response." });
  ...
  it("passes valid entry arrays to summary tasks", async () => {
    const runAiLibrarianTask = vi.fn(
      async () =>
        ({
          text: "Summary",
          provider: "mock",
