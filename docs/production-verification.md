...
- To verify live AI, set `MEMORA_AI_PROVIDER=google`, `MEMORA_AI_MODEL`, and `GEMINI_API_KEY` or `GOOGLE_API_KEY`, then create a memory and confirm the AI title/reflection are generated through the server API without exposing keys in browser payloads.
