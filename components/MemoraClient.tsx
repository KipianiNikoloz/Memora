...
export function MemoraProvider({ children, runtimeMode, supabaseClient }: MemoraProviderProps) {
  ...
  useEffect(() => {
    if (mode !== "demo") return;
    const storedUser = readStoredJson<UserProfile | null>(userKey, demoUser, isNullable(isUserProfile));
    const storedEntries = readStoredJson<MemoryEntry[]>(entriesKey, seedEntries, isMemoryEntryArray);
    const storedBadges = readStoredJson<MilestoneBadge[]>(badgesKey, [], isMilestoneBadgeArray);
    const storedBadgeWallet = readStoredJson<DemoBadgeWallet | null>(
      badgeWalletKey,
      null,
      isNullable(isDemoBadgeWallet),
    );
    setUser(storedUser);
  ...
  useEffect(() => {
    ...
    if (!supabase) {
      setError(
        "Supabase mode is enabled, but `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` are required.",
      );
      setHydrated(true);
      setLoading(false);
      return;
  ...
  const saveBadge = useCallback(
    async (badge: MilestoneBadge): Promise<MilestoneBadge> => {
      if (mode === "supabase") {
